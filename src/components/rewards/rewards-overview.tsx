    ...
    <div className="grid gap-4 lg:grid-cols-2" data-testid="rewards-overview-card">
      ...
      <div className="rounded-xl border border-border bg-card p-4">
        ...
        <button
          onClick={onClaim}
          disabled={!canClaim}
          className="mt-4 w-full rounded-lg bg-cta px-4 py-2.5 text-sm font-medium text-cta-fg transition-colors hover:bg-cta-hover disabled:cursor-not-allowed disabled:opacity-50"
        >
          {!rewards.rewards_enabled
          ...
          <div className="flex items-start gap-2">
            <SettlementIcon
              className={`mt-0.5 h-4 w-4 ${
                settlementBlocked ? 'text-amber-700' : settlementCommitted ? 'text-emerald-700' : 'text-blue-700'
              }`}
            />
            <div>
              <p
                className={`text-sm font-medium ${
                  settlementBlocked ? 'text-amber-900' : settlementCommitted ? 'text-emerald-900' : 'text-blue-900'
                }`}
              >
              ...
              <p
                className={`mt-1 text-xs ${
                  settlementBlocked ? 'text-amber-800' : settlementCommitted ? 'text-emerald-800' : 'text-blue-800'
                }`}
              >
