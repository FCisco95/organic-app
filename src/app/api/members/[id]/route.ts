...
export async function GET(_request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    const { data, error } = await supabase
      .from('user_profiles')
      .select(
        'id, name, email, avatar_url, organic_id, role, total_points, tasks_completed, profile_visible, bio, location, website, twitter, discord, wallet_pubkey, created_at, easter_2026_egg_elements'
      )
      .eq('id', id)
      .maybeSingle();
