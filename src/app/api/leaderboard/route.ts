...
const LEADERBOARD_COLUMNS =
  'id, name, organic_id, avatar_url, total_points, tasks_completed, role, rank, dense_rank, xp_total, level, current_streak, restriction_status, easter_2026_egg_elements';
...
function normalizeLeaderboardRow(row: LeaderboardRow): NormalizedLeaderboardEntry | null {
  ...
  return {
    ...
    easter_2026_egg_elements: row.easter_2026_egg_elements ?? [],
  };
}
...
async function directUserProfilesFallback(): Promise<LeaderboardRow[]> {
  ...
  const { data, error } = await supabase
    .from('user_profiles')
    .select('id, name, organic_id, avatar_url, total_points, tasks_completed, role, xp_total, level, current_streak, restriction_status, easter_2026_egg_elements')
    .not('organic_id', 'is', null)
    .order('xp_total', { ascending: false })
    .order('total_points', { ascending: false })
    .limit(100);
